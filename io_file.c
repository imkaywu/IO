...
p_ply ply_header_read(char *fname, int *num_vertices, int *num_faces, int *num_range_grid)
{
...
int ply_read_1(char *fname, double *pt, double *n, double *c, double *f, double *g)
{
    int num_vertices;
    int num_grids;
    ...
    {
        ply_set_read_cb(ply, "face", "vertex_indices", read_list_cb, NULL, 0);
    }
...
{
    long length, ind_elem;
    ply_get_argument_property(argument, NULL, &length, &ind_elem);
...
//        grid[ind_instance] = -1;
    ...
    // For face, no yet
    if (ind_elem != -1)
        face[DIM * ind_instance + ind_elem] = val;
    
    return 1; // restricted by the RPly library
}

int ply_write_list(char *fname, double *g, int row, int col)
{
...
int ply_write_1(char *fname, double *pt, double *n, double *c, double *f, double *g, int num_vertices, int num_faces, int row, int col)
{
    p_ply ply = ply_create(fname, PLY_ASCII, NULL, 0, NULL);
